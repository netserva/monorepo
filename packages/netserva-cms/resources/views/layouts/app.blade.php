                                        ...
                                        <div class="py-1">
                                            ...
                                            @foreach($children as $child)
                                                <a href="{{ $child['url'] }}"
                                                   ...
                                                   @if($child['new_window'] ?? false) target="_blank" rel="noopener noreferrer" @endif>
                                                    {{ $child['label'] }}
                                                </a>
                                            @endforeach
                                        </div>
                ...
                @if($mobileMenu)
                    @foreach($mobileMenu->getMenuItems() as $item)
                        @php
                            ...
                            $hasMobileChildren = is_array($mobileChildren) && count($mobileChildren) > 0;
                        @endphp
                        ...
                        @if($hasMobileChildren)
                            @foreach($mobileChildren as $child)
                                <a href="{{ $child['url'] }}"
                                   class="block px-3 py-2 pl-6 rounded-md text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
                                   @if($child['new_window'] ?? false) target="_blank" rel="noopener noreferrer" @endif>
                                    └ {{ $child['label'] }}
                                </a>
                            @endforeach
