...
class AddZoneCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        if (! $kind || ! in_array($kind, ['Native', 'Master', 'Secondary', 'Slave'])) {
            $kind = select(
                label: 'Zone kind',
                options: [
                    'Native' => 'Native (PowerDNS native replication)',
                    'Master' => 'Master (AXFR master)',
                    'Secondary' => 'Secondary (AXFR slave)',
                ],
                default: 'Native'
            );
        }
        ...
        if (in_array($kind, ['Secondary', 'Slave']) && ! $masters) {
            $mastersInput = text(
